...
public class JohnHopkinsStrategy implements IndianDiseaseStat {
	...
	public String GetActiveCount() {
		...
		try {
			...
			List<JohnHopkinResponse> indianJohnHopkinResponse = Arrays.stream(johnHopkinResponses).filter(response -> response.getCountry().equals("India")).collect(Collectors.toList());

			List<Integer> confirmedCases = indianJohnHopkinResponse.stream().map(response -> (int) Math.round(response.getStats().getConfirmed())).collect(Collectors.toList());

			int sumOfConfirmedCases = confirmedCases.stream().reduce(0, (subtotal, element) -> subtotal + element);

			return String.valueOf(sumOfConfirmedCases);
